...
class Receiver:
    ...
    async def receive_frame(self, frame):
        ...
        if frame_number == self.RN:
            ...
            # self.data_buffer[frame_number] = data
            dynamic_insert(self.data_buffer, frame_number, data)
            # print("self.data_buffer",self.data_buffer)
            ...
            if header[2]:
                ...
                try:
                    ...
                    image = Image.open(io.BytesIO(b''.join(filter(None, self.data_buffer))))
                    image.save("out.jpg")
                ...
                with open("out.txt", "w") as file:
                    file.write(b''.join(filter(None, self.data_buffer)).decode('utf-8', errors='replace'))
